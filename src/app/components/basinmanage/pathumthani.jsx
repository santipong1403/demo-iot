...
// ปรับ x, y ให้ตรงกับตำแหน่ง ปตร.สน.บางบัวทอง ในรูป pathumthaniflow.jpg
const initialArrows = [
  {
    id: 1,
    x: 37.6,   // % จากซ้าย — ตำแหน่ง ปตร.สน.บางบัวทอง ในผังน้ำ
    y: 92.8,   // % จากบน
    rotation: 0,
    scale: 1,
    type: "pier",
...
const MOCK_PIER = [
  {
    station_code:   "PT.1",
    station_detail: "สน.ปตร.บางบัวทอง",
    brae_level:     2.80,
    wl_values:      1.12,
    wl_down:        0.85,
    open_gate:      2,
    ...
    basin:          "เจ้าพระยา",
    region:         "ภาคกลาง",
    office:         "โครงการส่งน้ำและบำรุงรักษาปทุมธานี",
    lat:            13.9118,
    lng:            100.4312,
    build_year:     "2540",
    complete_year:  "2542",
    ...
    additional_canal: "คลองบางบัวทอง → คลองพระยาบรรลือ → แม่น้ำเจ้าพระยา",
    remark:
      "ควบคุมการระบายน้ำพื้นที่ลุ่มน้ำปทุมธานีฝั่งตะวันตก ระดับน้ำเหนือ-ท้ายอยู่ในเกณฑ์ปกติ เปิดบาน 2 บาน รับน้ำจากคลองพระพิมล",
    series: {
      level: [
...
function IconPin() {
  return (
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#374151" strokeWidth="1.8" strokeLinecap="round">
...
// ─── LINE CHART ───────────────────────────────────────────────────────────────
function LineChart({ datasets, labels, height = 110 }) {
      ...
      {yTicks.map((v, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={padL - 4} y={y + 4} fontSize={8} fill="#9ca3af" textAnchor="end">
              {v.toFixed(2)}
            </text>
          </g>
        );
      ...
      {datasets.map((ds, di) => {
        ...
        return (
          <g key={di}>
            ...
            <polyline points={p} fill="none" stroke={ds.color} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />
          </g>
        );
      })}
      {labels
        ...
        .map((l, i) => {
          ...
          return (
            <text key={i} x={x} y={H - 4} fontSize={8} fill="#9ca3af" textAnchor="middle">
              {l}
            </text>
          );
        })}
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height }}>
      ...
      {HOURS.filter((h) => h % 4 === 0).map((h) => {
        ...
        return (
          <text key={h} x={x} y={H - 4} fontSize={8} fill="#9ca3af" textAnchor="middle">
...
function Popup({ data, onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* HEADER */}
        <div style={{ padding: "14px 18px", display: "flex", alignItems: "center", gap: 12, borderBottom: "1px solid #f1f5f9", flexShrink: 0, background: "#fafafa" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>
              ประตูระบายน้ำ · จ.{data.province_t}
            </div>
          ...
          <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: 7, border: "1px solid #e5e7eb", background: "#fff", cursor: "pointer", fontSize: 16, color: "#94a3b8", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            ×
          </button>
        </div>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid #f1f5f9", flexShrink: 0, background: "#fff", padding: "0 4px" }}>
          {[["water", "ข้อมูลน้ำ"], ["building", "ข้อมูลอาคาร"]].map(([id, label]) => (
            <button key={id} onClick={() => setTab(id)} style={{ padding: "10px 20px", fontSize: 13, fontWeight: tab === id ? 700 : 500, color: tab === id ? accentColor : "#6b7280", border: "none", borderBottom: tab === id ? `2px solid ${accentColor}` : "2px solid transparent", background: "none", cursor: "pointer", fontFamily: "'Sarabun',sans-serif" }}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "14px 16px", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "10px 14px", fontSize: 12, color: "#64748b", lineHeight: 1.7 }}>
            ประตูระบายน้ำ {data.station_detail} ทำหน้าที่ควบคุมและระบายน้ำ ตำบล{data.subdistrict} อำเภอ{data.amphoe} จังหวัด{data.province_t}
          </div>
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div style={{ background: "#f8fafc", borderRadius: 8, padding: 12, border: "1px solid #e2e8f0" }}>
                  ...
                  <LineChart datasets={[{ data: seriesLevel, color: "#1d4ed8" }]} labels={HOURS.map((h) => `${String(h).padStart(2, "0")}:00`)} height={110} />
                </div>
                <div style={{ background: "#f8fafc", borderRadius: 8, padding: 12, border: "1px solid #e2e8f0" }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: "#374151", marginBottom: 6, display: "flex", alignItems: "center", gap: 5 }}>
              ...
              {/* EXTRA INFO */}
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "#94a3b8", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, overflow: "hidden" }}>
                  {[
                    ["ระดับตลิ่ง",      `${data.brae_level} ม.รทก.`],
                    ["อำเภอ / จังหวัด", `${data.amphoe} / ${data.province_t}`],
                  ].map(([label, val], i, arr) => (
                    <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 14px", borderBottom: i < arr.length - 1 ? "1px solid #f8fafc" : "none", fontSize: 12 }}>
                      ...
                      <span style={{ fontWeight: 600, fontFamily: "'IBM Plex Mono',monospace", fontSize: 13, color: "#0f172a" }}>{val}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {tab === "building" && (
            <>
              ...
              <SectionBox icon={<IconGate />} title="ข้อมูลประตูระบายน้ำ">
                <InfoGrid rows={[
                  ["จำนวนบานประตู",    `${data.gate_count} บาน`],
                  ["ประเภทบาน",        data.gate_type],
                  ["ความกว้าง/บาน",    `${data.gate_width} ม.`],
                  ["ความสูง/บาน",      `${data.gate_height} ม.`],
                  ["อัตราระบายสูงสุด", `${data.max_discharge} ม³/วิ`],
                  ["ระดับน้ำล้นตลิ่ง", `${data.flood_level} ม.รทก.`],
                  ["ระดับน้ำปกติ",     `${data.normal_level} ม.รทก.`],
                  ["ระดับน้ำล้นฝาย",  `${data.spill_level} ม.รทก.`],
                ]} />
              </SectionBox>
...
// ─── STATION ICON (SVG จาก Figma) ────────────────────────────────────────────
function StationIcon({ arrow, onClick }) {
const size = arrow.size === "small" ? 36 : arrow.size === "large" ? 56 : 44;  
const half = size / 2;

  // SVG ตัวจริงจาก Figma — วงกลมสีน้ำเงิน + ไอคอนประตูระบายน้ำ
  // ใช้ foreignObject เพราะต้องการ drop-shadow filter ที่ซับซ้อน
  const svgStr = `
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 80 80" fill="none"
     style="filter:drop-shadow(1px 1px 3px rgba(0,0,0,0.39));display:block;cursor:pointer;">
  <!-- วงกลมพื้นหลังสีขาว + วงแหวนสีน้ำเงิน -->
  <circle cx="40" cy="40" r="34" fill="white"/>
  <circle cx="40" cy="40" r="29" stroke="#3D00B6" stroke-width="10"/>
  <!-- ไอคอนประตูระบายน้ำ -->
  <path d="M14.4141 53.919H17.3074L65.0215 53.4513V47.007L61.1813 23.8281V29.389H19.0434V23.8281L14.4141 47.007V53.919Z" fill="#AAB8CD"/>
  <rect x="20.9297" y="26.3506" width="38.4028" height="2.84075" fill="black"/>
  <path d="M20.9824 40.1853V29.2432H58.9643V40.1853H20.9824Z" fill="#0833B1" stroke="#2E54A4"/>
  <path d="M19.2997 47.2889L20.6149 44.5534L20.9305 26.1937H26.875V39.9766H28.9793V26.1937H35.0291V39.9766H37.0807V26.1937H42.8674V39.9766H44.9191V26.1937H50.9688V39.9766H53.0205V26.1937H59.1754L59.4385 44.5534L60.5432 47.2889H62.8579L61.5953 44.5534L61.0167 23.9316H59.1754V24.8786H53.0205V23.9316H50.9688V24.8786H44.9191V23.9316H42.8674V24.8786H37.0807V23.9316H35.0291V24.8786H28.9793V23.9316H26.875V24.8786H20.9305V23.9316H19.0367L18.5106 44.5534L17.248 47.2889H19.2997Z" fill="#CCDBF9" stroke="#CCDBF9"/>
  <path d="M21.0341 40.1377L20.6133 42.3472H59.3843L59.016 40.1377H21.0341Z" fill="#3C75FF" stroke="#3C75FF"/>
  <path d="M20.5774 44.7676L19.4727 49.8178H61.0844L59.4536 45.0832V44.7676H20.5774Z" fill="#3C75FF" stroke="#3C75FF"/>
  <rect x="19.4727" y="49.7158" width="41.6644" height="4.31373" fill="#0833B1"/>
  <path d="M20.6133 44.6573V42.29H59.4369V44.6573H20.6133Z" fill="#0833B1" stroke="#3C75FF"/>
  <path d="M19.3522 47.3936H17.1953V54.1272H19.3522V47.3936Z" fill="#BECCE6" stroke="#CCDBF9"/>
  <path d="M62.8737 47.3936H60.7168V54.1272H62.8737V47.3936Z" fill="#BECCE6" stroke="#CCDBF9"/>
</svg>`;
